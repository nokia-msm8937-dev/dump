...
CMD=`getprop vendor.wbg.bt`
TESTITEM=`getprop vendor.wbg.bt.testitem`
PRO_CHANNEL=`getprop vendor.wbg.bt.channel`
PRO_PAYLOAD=`getprop vendor.wbg.bt.payload`
PRO_POWER=`getprop vendor.wbg.bt.power`
PRO_SLOT=`getprop vendor.wbg.bt.slot`
RX_DATA=`getprop vendor.wbg.bt.package`
LE_PACKAGE=`getprop vendor.wbg.bt.lepackage`

RX_LOG_FILE="/data/data/com.fihtdc.wbgtesttool/rx.txt"
RX_HCI_FILE="/data/data/com.fihtdc.wbgtesttool/rx_hci.txt"
LE_RX_LOG_FILE="/data/data/com.fihtdc.wbgtesttool/lerx.txt"
LE_RX_HCI_FILE="/data/data/com.fihtdc.wbgtesttool/lerx_hci.txt"
...
function Bluetooth_GetRxdata(){
	btconfig /dev/smd3 rawcmd 3F 04 02 > $RX_LOG_FILE

	HCI_BIT="0"
	# take out RX hci data
	busybox cat $RX_LOG_FILE | busybox grep '\[ff\]' | busybox sed 's/\[//g' | busybox sed 's/\]/ /g' > $RX_HCI_FILE

	HCI_RAW_DATA_CH1=$(busybox printf "%d" $(busybox awk 'NR==1{ print "0x"$9 $8 $7 $6 } ' $RX_HCI_FILE))
	HCI_RAW_DATA_CH2=$(busybox printf "%d" $(busybox awk 'NR==1{ print "0x"$41 $40 $39 $38 } ' $RX_HCI_FILE))
	HCI_RAW_DATA_CH3=$(busybox printf "%d" $(busybox awk 'NR==2{ print "0x"$9 $8 $7 $6 } ' $RX_HCI_FILE))
	HCI_RAW_DATA_CH4=$(busybox printf "%d" $(busybox awk 'NR==2{ print "0x"$41 $40 $39 $38 } ' $RX_HCI_FILE))
	HCI_RAW_DATA_CH5=$(busybox printf "%d" $(busybox awk 'NR==3{ print "0x"$10 $9 $8 $7 } ' $RX_HCI_FILE))
	TOTAL_HCI_RAW_DATA=`busybox expr $HCI_RAW_DATA_CH1 + $HCI_RAW_DATA_CH2 + $HCI_RAW_DATA_CH3 + $HCI_RAW_DATA_CH4 + $HCI_RAW_DATA_CH5`

	RESET_RAW_DATA_CH1=$(busybox printf "%d" $(busybox awk 'NR==1{ print "0x"$13 $12 $11 $10 } ' $RX_HCI_FILE))
	RESET_RAW_DATA_CH2=$(busybox printf "%d" $(busybox awk 'NR==1{ print "0x"$45 $44 $43 $42 } ' $RX_HCI_FILE))
	RESET_RAW_DATA_CH3=$(busybox printf "%d" $(busybox awk 'NR==2{ print "0x"$13 $12 $11 $10 } ' $RX_HCI_FILE))
	RESET_RAW_DATA_CH4=$(busybox printf "%d" $(busybox awk 'NR==2{ print "0x"$45 $44 $43 $42 } ' $RX_HCI_FILE))
	RESET_RAW_DATA_CH5=$(busybox printf "%d" $(busybox awk 'NR==3{ print "0x"$14 $13 $12 $11 } ' $RX_HCI_FILE))
	...
	# setprop "total bit value" to vendor.wbg.bt.rxreport
	setprop vendor.wbg.bt.rxreport $TOTAL_HCI_BIT
	# delete capture rx_*.txt file
	rm $RX_LOG_FILE
}

function Bluetooth_GetLeRxPacketCount(){
	btconfig /dev/smd3 rawcmd 08 1F > $LE_RX_LOG_FILE
	sleep 0.5
	# take out LE hci data
	busybox cat $LE_RX_LOG_FILE | busybox grep '\[e\]' | busybox sed 's/\[//g' | busybox sed 's/\]/ /g' > $LE_RX_HCI_FILE
	# parse le package size
	TOTAL_PACKET_BIT=$(busybox printf "%d" $(busybox awk 'NR==1{ print "0x"$8 $7 } ' $LE_RX_HCI_FILE))
	# setprop "total bit value" to vendor.wbg.bt.rxreport
	setprop vendor.wbg.bt.lerxreport $TOTAL_PACKET_BIT
	# delete capture rx_*.txt file
	rm $LE_RX_LOG_FILE
}
...
function Bluetooth_Test(){
	if [ "$CMD" == "btstart" ]; then
		HCI_Rest
		sleep 1.5
		hci_qcomm_init -e && setprop bt.module.on 0 || setprop bt.module.on 1
		sleep 1
		# read BD_ADDR
